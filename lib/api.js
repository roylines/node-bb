...
api.users.create = function(request, response) {
  ...
  users.validate.username(request.body.username, function(e) {
    if(e) {
      errors.username = e;
      hasErrors = true;
    }
    users.validate.email(request.body.email, function(e) {
      if(e) {
        errors.email = e;
        hasErrors = true;
      }
...
/*
api.cycletimes.get = function() {
  var cycleTimes = [{
    period: 'this week',
    shirts: [ { size: 'small', average: 1, error: 1, predictability: 'poor' }]
  }];
};
*/
module.exports = api;
